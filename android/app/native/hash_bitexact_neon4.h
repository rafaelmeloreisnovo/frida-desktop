...
#ifndef RAFAELIA_HASH_NEON4_FULL_UNROLL
#define RAFAELIA_HASH_NEON4_FULL_UNROLL 0
#endif
#if RAFAELIA_HASH_NEON4_FULL_UNROLL && defined(__clang__)
#define RHU _Pragma("clang loop unroll(full)")
#else
#define RHU
#endif
...
RAFAELIA_FORCE_INLINE void rafaelia_sha256x4_init(rafaelia_sha256x4_state *q){
 ...
 unsigned int i,j;RHU for(i=0;i<8u;i++){RHU for(j=0;j<4u;j++)q->h[i][j]=x[i];}
}
...
RAFAELIA_FORCE_INLINE void rafaelia_sha256x4_compress(rafaelia_sha256x4_state *q,const rafaelia_u8 *p0,const rafaelia_u8 *p1,const rafaelia_u8 *p2,const rafaelia_u8 *p3){
 ...
 RHU for(i=0;i<16u;i++)w[i]=(rafaelia_v4u32){rafaelia_load32_be(p0+(i<<2)),rafaelia_load32_be(p1+(i<<2)),rafaelia_load32_be(p2+(i<<2)),rafaelia_load32_be(p3+(i<<2))};
 RHU for(i=16u;i<64u;i++)w[i]=RVSS1(w[i-2])+w[i-7]+RVSS0(w[i-15])+w[i-16];
 ...
 RHU for(i=0;i<64u;i++){t1=h+RVBS1(e)+RVCH(e,f,g)+RV4(k[i])+w[i];t2=RVBS0(a)+RVMAJ(a,b,c);h=g;g=f;f=e;e=d+t1;d=c;c=b;b=a;a=t1+t2;}
 {rafaelia_v4u32 z[8];z[0]=a+(rafaelia_v4u32){q->h[0][0],q->h[0][1],q->h[0][2],q->h[0][3]};z[1]=b+(rafaelia_v4u32){q->h[1][0],q->h[1][1],q->h[1][2],q->h[1][3]};z[2]=c+(rafaelia_v4u32){q->h[2][0],q->h[2][1],q->h[2][2],q->h[2][3]};z[3]=d+(rafaelia_v4u32){q->h[3][0],q->h[3][1],q->h[3][2],q->h[3][3]};z[4]=e+(rafaelia_v4u32){q->h[4][0],q->h[4][1],q->h[4][2],q->h[4][3]};z[5]=f+(rafaelia_v4u32){q->h[5][0],q->h[5][1],q->h[5][2],q->h[5][3]};z[6]=g+(rafaelia_v4u32){q->h[6][0],q->h[6][1],q->h[6][2],q->h[6][3]};z[7]=h+(rafaelia_v4u32){q->h[7][0],q->h[7][1],q->h[7][2],q->h[7][3]};RHU for(i=0;i<8u;i++){RHU for(j=0;j<4u;j++)q->h[i][j]=z[i][j];}}
}

RAFAELIA_FORCE_INLINE void rafaelia_sha256x4_oneblock(const rafaelia_u8 *p0,const rafaelia_u8 *p1,const rafaelia_u8 *p2,const rafaelia_u8 *p3,rafaelia_u32 out[8][4]){rafaelia_sha256x4_state q;unsigned int i,j;rafaelia_sha256x4_init(&q);rafaelia_sha256x4_compress(&q,p0,p1,p2,p3);RHU for(i=0;i<8u;i++){RHU for(j=0;j<4u;j++)out[i][j]=q.h[i][j];}}
