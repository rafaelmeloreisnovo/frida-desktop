...
The resulting object must have zero undefined symbols and its disassembly must contain NEON vector add/XOR/shift/OR operations.

## Loop specialization

`RAFAELIA_HASH_NEON4_FULL_UNROLL=0` is the default portable specialization.

`RAFAELIA_HASH_NEON4_FULL_UNROLL=1` asks Clang to fully expand the fixed SHA-256 schedule/round loops. CI requires the ARM32 full-unroll probe to contain zero conditional loop branches and to remain semantically equivalent to the default path.
...
- physical NEON width: 128 bits;
- byte lanes per vector operation: 16;
- vectors per 4096-byte page: 256;
- caller owns the 4096-byte buffers;
- the transport leaf has three independent source streams where the existing `xor3` ABI is used;
- `pld` is a cache prefetch hint, not a cache-residency guarantee.

The intended boundary is:

`memory -> PLD/prefetch hint -> cache hierarchy -> caller buffer -> bit-exact word/SIMD math -> caller output buffer`

The software can choose addresses and prefetch points, but it cannot directly command the hardware cache replacement policy. Cache miss rate therefore remains an observed metric, not a software-controlled fact.

## Eight cores

The strict math and transport leaves contain no scheduler or thread dependency. Static partitioning across 1/2/4/8 workers belongs to the external measurement/orchestration layer already present in the repository. This preserves the freestanding leaf while allowing the exact object to be measured on the eight-core ARM device.

## Evidence boundary

The structural/semantic gates may prove:

- exact digest equivalence for tested inputs;
- zero production-header global symbols;
- zero undefined symbols in ARM32 probes;
- actual NEON instructions in the ARM32 object;
- four parallel SHA-256 `u32` lanes per 128-bit vector;
- sixteen parallel transport `u8` lanes per 128-bit vector;
- zero conditional loop branches in the full-unroll probe.

They do **not** prove physical cache behavior or a performance ratio. Until an exact-object physical device run binds object SHA, CPU identity, clock/governor, alignment, warm/cold protocol and repeated samples, the following remain `TOKEN_VAZIO`:

- physical ARM32 execution of this new hash specialization;
- cache miss rate;
- throughput ratio;
- physical DRAM bandwidth ratio;
- generalized 3x throughput/bandwidth;
- synchronized eight-core wall scaling.
