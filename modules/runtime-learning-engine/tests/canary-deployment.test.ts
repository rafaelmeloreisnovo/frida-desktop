...
describe('Phase 4.2: Canary Deployment & Auto-Rollback', () => {
  ...
  describe('Phase 4.2.1: Canary Deployment Orchestration', () => {
    ...
    test('Each stage has correct monitoring duration', () => {
      const expectedDurations = [60, 120, 240, 1440]; // minutes
  ...
  describe('Phase 4.2.2: Stage Metrics Evaluation', () => {
    ...
    test('Low success rate triggers rollback decision', async () => {
      ...
      const poorMetrics: StageMetrics = {
        successRate: 65, // Below 70% threshold for stage 1
        errorRate: 45,
        errorCount: 45,
    ...
    test('High error rate triggers rollback decision', async () => {
      ...
      const errorMetrics: StageMetrics = {
        successRate: 75,
        errorRate: 60, // Above 50/hour threshold for stage 1
        errorCount: 60,
        memoryGrowthMBPerMin: 2.0,
    ...
    test('Excessive memory growth triggers halt decision', async () => {
      ...
      const memoryMetrics: StageMetrics = {
        ...
        errorRate: 30,
        errorCount: 30,
        memoryGrowthMBPerMin: 15, // Above 10 MB/min threshold
        averageBugCaptureLatency: 50,
  ...
  describe('Phase 4.2.3: Stage Advancement', () => {
    ...
    test('Cannot advance if current stage not completed', async () => {
      ...
      expect(plan.currentStage).toBe(0); // Still at stage 1
  ...
  describe('Phase 4.2.4: Automatic Rollback', () => {
    test('Rollback automation detects success rate trigger', () => {
      const metrics = {
        fix_success_rate: 65,    // Below critical 70%
        errors_per_hour: 30,
        memory_growth_mb_per_min: 2,
    ...
    test('Rollback automation detects error rate trigger', () => {
      const metrics = {
        fix_success_rate: 85,
        errors_per_hour: 60,     // Above critical 50/hour
        memory_growth_mb_per_min: 2,
        rollback_success_rate: 95,
        corruption_count: 0,
        watchdog_state: 1,
    ...
    test('Rollback automation detects memory growth trigger', () => {
      const metrics = {
        fix_success_rate: 85,
        errors_per_hour: 30,
        memory_growth_mb_per_min: 12, // Above warning 10 MB/min
        rollback_success_rate: 95,
        corruption_count: 0,
        watchdog_state: 1,
        storage_used_mb: 400,
    ...
    test('Rollback automation detects corruption trigger', () => {
      const metrics = {
        ...
        memory_growth_mb_per_min: 2,
        rollback_success_rate: 95,
        corruption_count: 1,      // Should be 0
        watchdog_state: 1,
        storage_used_mb: 400,
        bug_capture_latency_ms: 50
      };
    ...
    test('Rollback automation correctly disables disabled triggers', () => {
      const customTriggers = [...DEFAULT_ROLLBACK_TRIGGERS];
      customTriggers[0].enabled = false; // Disable success rate trigger

      const automation2 = new RollbackAutomation('test2', customTriggers);

      const metrics = {
        fix_success_rate: 65,    // Would trigger if enabled
        errors_per_hour: 30,
        memory_growth_mb_per_min: 2,
        rollback_success_rate: 95,
        corruption_count: 0,
      ...
      // Should not trigger because success rate check is disabled
      // and no other metrics exceed thresholds
    ...
    test('Rollback automation tracks multiple events', async () => {
      const event1 = await automation.executeRollback('Event 1');
      const event2 = await automation.executeRollback('Event 2');
